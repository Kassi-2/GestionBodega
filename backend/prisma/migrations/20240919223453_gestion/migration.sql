/*
  ...
  - You are about to alter the column `state` on the `product` table. The data in that column could be lost. The data in that column will be cast from `VarChar(191)` to `TinyInt`.
  - A unique constraint covering the columns `[name]` on the table `product` will be added. If there are existing duplicate values, this will fail.

*/
-- AlterTable
ALTER TABLE `product` ADD COLUMN `fungible` BOOLEAN NOT NULL DEFAULT false,
    MODIFY `description` VARCHAR(191) NULL,
    MODIFY `stock` INTEGER NULL DEFAULT 0,
    MODIFY `criticalStock` INTEGER NOT NULL DEFAULT 1,
    MODIFY `state` BOOLEAN NOT NULL DEFAULT true;
